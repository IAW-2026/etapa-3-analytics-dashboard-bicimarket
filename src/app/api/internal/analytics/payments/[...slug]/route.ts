...
export async function GET(
  ...
  const token = process.env.DASHBOARD_TO_PAYMENTS_SERVICE_TOKEN

  if (!baseUrl) {
    return NextResponse.json(
      { error: { code: "UPSTREAM_ERROR", message: "PAYMENTS_API_URL no está configurada" } },
      { status: 503 },
    )
  }
  if (!token) {
    return NextResponse.json(
      { error: { code: "UPSTREAM_ERROR", message: "DASHBOARD_TO_PAYMENTS_SERVICE_TOKEN no está configurado" } },
      { status: 503 },
    )
  }
